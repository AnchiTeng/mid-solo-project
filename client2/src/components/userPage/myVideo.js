...
const MyVideo = ()=> {
  ...
  //     console.log('just before the try catch block');
  //     try{
  ...
  //     } catch(err) {
  //       console.log('there was error with the request')
  ...
  const onSubmit = async e => {
    ...
    e.preventDefault();
      ...
      try{
        console.log('just before the post request');
        const res = await axios.post('/myvideo', {} ,{
            headers: {
                'Content-Type': 'multipart/form-data'
              },
  ...
  return (
    <Fragment>
      ...
      <form>
        ...
        <button onClick={onSubmit}>Press here</button>
        {/* <input
          type='submit'
          value='anything Upload'
